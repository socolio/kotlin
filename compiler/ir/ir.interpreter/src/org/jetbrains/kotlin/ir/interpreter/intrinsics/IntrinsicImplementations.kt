...
internal object EmptyArray : IntrinsicBase() {
    ...
    override fun evaluate(irFunction: IrFunction, environment: IrInterpreterEnvironment) {
        val returnType = environment.callStack.getStateBySymbol(irFunction.symbol) as KTypeState
...
internal object ArrayOf : IntrinsicBase() {
    ...
    override fun evaluate(irFunction: IrFunction, environment: IrInterpreterEnvironment) {
        ...
        val varargVariable = environment.callStack.getStateBySymbol(elementsSymbol)
...
internal object ArrayOfNulls : IntrinsicBase() {
    ...
    override fun evaluate(irFunction: IrFunction, environment: IrInterpreterEnvironment) {
        val size = environment.callStack.getStateBySymbol(irFunction.valueParameters.first().symbol).asInt()
        val array = arrayOfNulls<Any?>(size)
        val typeArgument = irFunction.typeParameters.map { environment.callStack.getStateBySymbol(it.symbol) }.single() as KTypeState
...
internal object EnumValues : IntrinsicBase() {
    ...
    private fun getEnumClass(irFunction: IrFunction, environment: IrInterpreterEnvironment): IrClass {
        return when (irFunction.fqNameWhenAvailable.toString()) {
            "kotlin.enumValues" -> {
                val kType = environment.callStack.getStateBySymbol(irFunction.typeParameters.first().symbol) as KTypeState
...
internal object EnumValueOf : IntrinsicBase() {
    ...
    private fun getEnumEntryByName(irFunction: IrFunction, environment: IrInterpreterEnvironment): IrEnumEntry? {
        val enumClass = getEnumClass(irFunction, environment)
        val enumEntryName = environment.callStack.getStateBySymbol(irFunction.valueParameters.first().symbol).asString()
...
internal object EnumHashCode : IntrinsicBase() {
    ...
    override fun evaluate(irFunction: IrFunction, environment: IrInterpreterEnvironment) {
        val hashCode = environment.callStack.getStateBySymbol(irFunction.dispatchReceiverParameter!!.symbol).hashCode()
...
internal object JsPrimitives : IntrinsicBase() {
    ...
    override fun evaluate(irFunction: IrFunction, environment: IrInterpreterEnvironment) {
        when (irFunction.fqNameWhenAvailable.toString()) {
            "kotlin.Long.<init>" -> {
                val low = environment.callStack.getStateBySymbol(irFunction.valueParameters[0].symbol).asInt()
                val high = environment.callStack.getStateBySymbol(irFunction.valueParameters[1].symbol).asInt()
                environment.callStack.pushState((high.toLong().shl(32) + low).toState(irFunction.returnType))
            }
            "kotlin.Char.<init>" -> {
                val value = environment.callStack.getStateBySymbol(irFunction.valueParameters[0].symbol).asInt()
...
internal object ArrayConstructor : IntrinsicBase() {
    ...
    override fun unwind(irFunction: IrFunction, environment: IrInterpreterEnvironment): List<Instruction> {
        ...
        val size = environment.callStack.getStateBySymbol(sizeDescriptor).asInt()

        val initDescriptor = irFunction.valueParameters[1].symbol
        val initLambda = environment.callStack.getStateBySymbol(initDescriptor) as KFunctionState
    ...
    override fun evaluate(irFunction: IrFunction, environment: IrInterpreterEnvironment) {
        ...
        val type = environment.callStack.getStateBySymbol(irFunction.symbol) as KTypeState
...
internal object AssertIntrinsic : IntrinsicBase() {
    ...
    override fun unwind(irFunction: IrFunction, environment: IrInterpreterEnvironment): List<Instruction> {
        ...
        val messageLambda = environment.callStack.getStateBySymbol(irFunction.valueParameters.last().symbol) as KFunctionState
    ...
    override fun evaluate(irFunction: IrFunction, environment: IrInterpreterEnvironment) {
        val value = environment.callStack.getStateBySymbol(irFunction.valueParameters.first().symbol).asBoolean()
        if (value) return
