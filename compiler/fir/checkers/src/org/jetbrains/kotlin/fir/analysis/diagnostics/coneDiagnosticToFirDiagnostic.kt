...
private fun mapSystemHasContradictionError(
    ...
    for (error in diagnostic.candidate.system.errors) {
        addIfNotNull(error.toDiagnostic(source, qualifiedAccessSource))
    }
}.ifEmpty {
    listOfNotNull(
    ...
    is NewConstraintError -> {
        ...
        when (position) {
            is ExpectedTypeConstraintPosition<*> -> {
                val inferredType =
                    if (!lowerConeType.isNullableNothing)
                        ...
                        upperConeType.withNullability(ConeNullability.NULLABLE)
            ...
            is KnownTypeParameterConstraintPosition<*> -> {
                val conePosition = position as ConeKnownTypeParameterConstraintPosition
                val typeArgument = conePosition.typeArgument

                FirErrors.UPPER_BOUND_VIOLATED.on(
                    conePosition.sourceElement ?: qualifiedAccessSource ?: source,
                    typeArgument
                )
            }
            else -> null
