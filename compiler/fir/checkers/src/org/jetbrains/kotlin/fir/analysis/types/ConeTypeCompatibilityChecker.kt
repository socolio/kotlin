            ...
            for (superConcreteClass in queue.removeFirst().superClasses(session)) {
                when (isLeaf[superConcreteClass]) {
                    ...
                    false -> {
                        ...
                        isLeaf[superConcreteClass] = false
                        queue.addLast(superConcreteClass)
                    }
                }
            }
        }
    ...
    private fun MutableMap<FirTypeParameterRef, BoundTypeArguments>.collectTypeArgumentMapping(
        ...
        while (queue.isNotEmpty()) {
            ...
            for ((firTypeParameterRef, boundTypeArgument) in mapping) {
                this.collect(typeParameterOwner, firTypeParameterRef, boundTypeArgument, compatibilityUpperBound)
            }
        }
    }
